...
import { customerPromoPrompt } from "@/lib/ai/prompts";
...
const CustomerMarketingForm = ({ customer }: { customer: Customer }) => {
  ...
  const handleSubmit = async (values: z.infer<typeof customerFormSchema>) => {
    ...
    try {
      submit({
        ...
        Most Purchased Product: "${customer.mostPurchasedProductService ?? ""}"
        Product Description: "${values.productDescription}"
        Emojis: ${values.emojis ? '"Yes"' : '"No"'}
        Tone: "${values.tone}"
        `,
        additionalSystemPrompt: customerPromoPrompt,
      });
    } catch (error) {
...
export default CustomerMarketingForm;
